...
import { TOOLS, CATEGORY, CATEGORY_LABELS, EXAMPLE } from "../config.js";
...
export function buildCapabilities() {
  const categories = {};
  const capabilities = Object.entries(TOOLS).map(([key, t]) => {
...
export function buildPricing() {
  return {
    ...
    tools: Object.fromEntries(
      Object.entries(TOOLS).map(([k, t]) => [k, { path: t.path, price: t.price, free_per_day: t.freePerDay }])
    ),
  };
}
