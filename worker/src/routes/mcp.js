...
const REGISTRY = {
  ...
  guard_inspect: {
    key: "guard_inspect",
    description: "Screen untrusted content (web pages, emails, tool output) for prompt-injection/manipulation before your agent trusts it. Returns a risk score + reasons; a screening signal, not a guarantee.",
    schema: { type: "object", properties: { text: { type: "string", description: "Untrusted content to screen." }, deep: { type: "boolean", description: "Use the ML classifier too (default true)." } }, required: ["text"] },
    run: async (env, a) => json(await inspectContent(a.text, { deep: a.deep !== false }, env)),
  },
  guard_tx: {
    key: "guard_tx",
    description: "Pre-sign wallet firewall: decode a transaction and flag unlimited approvals, setApprovalForAll, scam addresses, and spend-cap breaches before signing. Returns a risk verdict + reasons.",
    schema: { type: "object", properties: { to: { type: "string" }, data: { type: "string", description: "Calldata (0x...)." }, value: { type: "string", description: "Native value in wei." }, chain: { type: "string" }, from: { type: "string" } }, required: ["to"] },
    run: async (env, a) => json(await inspectTransaction({ to: a.to, data: a.data, value: a.value, chain: a.chain, from: a.from }, env)),
  },
  check_domain: {
    key: "domain",
...
const BROWSER_MCP_TOOLS = new Set(["render_page", "screenshot_page"]);
    ...
    switch (method) {
      case "initialize":
        result = {
          ...
          instructions:
            "fetcher.cloud - the web capability layer for AI agents: extract and unfurl content, parse feeds, " +
            "resolve links, screen content/transactions with guard, and look up domains, DNS, SSL, and WHOIS. " +
            "Free daily tier; paid per call over x402 (USDC on Base) via the HTTP API.",
        };
        break;
      case "tools/list": {
        const brOk = !!(env && env.BROWSER);
        result = { tools: brOk ? TOOLS : TOOLS.filter((t) => !BROWSER_MCP_TOOLS.has(t.name)) };
        break;
      }
      case "tools/call":
